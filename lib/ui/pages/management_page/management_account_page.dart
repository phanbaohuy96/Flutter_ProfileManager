              ...
              SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, idx) {
                    return _buildGroupAccount(
                        idx == 0 ? null : accountGroups[idx - 1], idx);
                  },
                  itemCount: accountGroups.length + 1,
                ),
              ),
              DocListIndex(
                numItems: accountGroups.length + 1,
                ...
                child: _selectedIdx == 0
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, idx) {
                          return AccountItem(
                            account:
                                accountGroups[_selectedIdx - 1].accounts[idx],
                            backgroundColor: widget.mainColor,
                            groupImageAsset:
                                accountGroups[_selectedIdx - 1].imageAsset,
                          );
                        },
                        itemCount:
                            accountGroups[_selectedIdx - 1].accounts.length,
                      ),
              ),
            ),
          ),
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Text(
                  idx == 0
                      ? 'All'
                      : 'Count: ${group.accounts.length.toString()}',
  ...
  List<Widget> _buildAllAccountItem() {
    ...
    for (int i = 0; i < accountGroups.length; i++) {
      for (int j = 0; j < accountGroups[i].accounts.length; j++) {
        _accounts.add(
          AccountItem(
            account: accountGroups[i].accounts[j],
            backgroundColor: widget.mainColor,
            groupImageAsset: accountGroups[i].imageAsset,
          ),
        );
      }
    }
